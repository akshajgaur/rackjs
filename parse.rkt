...
(define (parse-define s)
  (match s
    [(list 'define (list-rest (? symbol? f) xs) e)
     (if (andmap symbol? xs) (Defn f xs (parse-e e)) (error "parse definition error"))]
...
(define (parse-e s)
  (match s
    ...
    [(list 'let (list (list (? symbol? x) e1)) e2) (Let x (parse-e e1) (parse-e e2))]
    [(cons e es) (App (parse-e e) (map parse-e es))]
    [_ (error "Parse error" s)]))

(define op0 '(read-byte peek-byte void))
...
(define op1
  '(add1 sub1
         ...
         string?
         string-length))
(define op2 '(+ - < = cons eq? make-vector vector-ref make-string string-ref))
(define op3 '(vector-set!))

(define (op? ops)
  (λ (x) (and (symbol? x) (memq x ops))))
